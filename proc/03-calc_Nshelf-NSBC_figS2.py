...
def poly_avg_plot(Nmean, Nstd, ssns, lons, lats, xis, yis, xy, axs, col="k", min_Nobs=20, N_annotations=True):
...
ax11 = fig.add_subplot(421, projection=proj)
ax12 = fig.add_subplot(422, projection=proj, sharex=ax11, sharey=ax11)
ax13 = fig.add_subplot(423, projection=proj, sharex=ax11, sharey=ax11)
ax14 = fig.add_subplot(424, projection=proj, sharex=ax11, sharey=ax11)
ax21 = fig.add_subplot(425, projection=proj, sharex=ax11, sharey=ax11)
ax22 = fig.add_subplot(426, projection=proj, sharex=ax11, sharey=ax11)
ax23 = fig.add_subplot(427, projection=proj, sharex=ax11, sharey=ax11)
ax24 = fig.add_subplot(428, projection=proj, sharex=ax11, sharey=ax11)
...
ax21.text(xtxt, ytxt, ssn, fontsize=18, fontweight="black", zorder=9, transform=ax21.transAxes)
...
ax22.text(xtxt, ytxt, ssn, fontsize=18, fontweight="black", zorder=9, transform=ax22.transAxes)
...
ax23.text(xtxt, ytxt, ssn, fontsize=18, fontweight="black", zorder=9, transform=ax23.transAxes)
...
ax24.text(xtxt, ytxt, ssn, fontsize=18, fontweight="black", zorder=9, transform=ax24.transAxes)
...
for csi, axi in zip([cs11, cs12, cs13, cs14], [ax11, ax12, ax13, ax14]):
    cbi = fig.colorbar(csi, cax=axi.inset_axes(cbax, zorder=10), orientation="horizontal", extend="both")
    cbi.ax.xaxis.set_ticks_position("top"); cbi.ax.xaxis.set_label_position("top"); cbi.ax.xaxis.set_ticks(np.linspace(vmi, vma, 4))
    cbi.set_label(cbname1, fontsize=15)

for csi, axi in zip([cs21, cs22, cs23, cs24], [ax21, ax22, ax23, ax24]):
    ...
    cbi.set_label(cbname2, fontsize=15)
...
def poly_avg_plot2(Nmean, Nstd, ssns, lons, lats, xis, yis, xy, axs, xoff, yoff, col="k", min_Nobs=20):
    xx, yy = np.flipud(xy[:, 0]), np.flipud(xy[:, 1])
    xs, ys = np.concatenate((xis, xx)), np.concatenate((yis, yy))
    xs, ys = np.append(xs, xs[0]), np.append(ys, ys[0])
    ps = np.array([(x0, y0) for x0, y0 in zip(xs, ys)])
    ...
    xt, yt = lonsp.mean() + xoff, latsp.mean() + yoff
    for ssn, axi in zip(ssns, axs):
        Nobstxt = "%d" % Nobs_ssn[ssn]
        axi.text(
            xt,
            yt,
...
ssn = "JFM"
cs11 = ax11.pcolormesh(x, y, Nnobs[ssn], vmin=nobsmin, vmax=nobsmax, cmap=cmap2)
ax11.text(xtxt, ytxt, ssn, fontsize=18, fontweight="black", zorder=9, transform=ax11.transAxes)

ssn = "AMJ"
cs12 = ax12.pcolormesh(x, y, Nnobs[ssn], vmin=nobsmin, vmax=nobsmax, cmap=cmap2)
ax12.text(xtxt, ytxt, ssn, fontsize=18, fontweight="black", zorder=9, transform=ax12.transAxes)

ssn = "JAS"
cs13 = ax13.pcolormesh(x, y, Nnobs[ssn], vmin=nobsmin, vmax=nobsmax, cmap=cmap2)
ax13.text(xtxt, ytxt, ssn, fontsize=18, fontweight="black", zorder=9, transform=ax13.transAxes)
...
#########################################
### Add polygons again just for plotting.
...
xoff, yoff = -0.3, -2.5
poly_avg_plot2(Nmean, Nstd, ssns, lons, lats, xis, yis, xy, axs, xoff, yoff, col="c")
